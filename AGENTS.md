...
### 原则 5：业务状态用 Workflow，不用 is_submittable

如果 DocType 有业务状态机（流转、审批），**不要**用 Frappe 的 `is_submittable: 1`。

- ❌ `is_submittable: 1` + Workflow → docstatus 锁定冲突
- ✅ 纯 Workflow + 所有 state 都 `doc_status: "0"` + 用 `allow_edit` 控制权限
